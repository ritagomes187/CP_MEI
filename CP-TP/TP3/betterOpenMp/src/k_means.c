...
// Associar os pontos aos clusters mais próximos 
 void associaPontos(Points *cluster, Points *pontos, int auxP[], int N, int K){
    //int flag = 0;
    int i; 
...
// Recalcula centroid de cada cluster
 ...
 void initPontosAux(int auxP[], int N){
    int v = N + 1; 
    #pragma omp parallel for
    for(int i = 0; i < N; i++){
        auxP[i] = v;
 ...
 void changePoints(Points *pontos,int pontosAux[],Points *clust,int N){
    int v = N + 1;
    for(int i = 0; i < N; i++){
        int novo = pontosAux[i];
        if (novo != v){
            int antigo = pontos[i].idC_size;
            pontos[i].idC_size = novo;
            clust[antigo].idC_size--;
            clust[novo].idC_size++;
        }
    }
 }
...
int main(int argc, char *argv[]){
    ...
    Points ponto;
     while (it < 20)
     {
        ...
        //printf( "oi\n"); 
        int *pontosAux = malloc(sizeof(int) * N);
        //printf( "oi\n"); 
        initPontosAux(pontosAux,N);
        //printf( "oi\n"); 
        associaPontos(clust,pontos,pontosAux,N,K);
        //printf( "oi\n"); 
        changePoints(pontos,pontosAux,clust,N);
        //printf( "oi\n"); 
        free(pontosAux);
        //ponto = clust[0];
        it++;
